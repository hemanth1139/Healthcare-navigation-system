  ...
  if (action.isPrimary) {
    return (
      <Link href={action.linkUrl} className="block group focus-ring rounded-2xl">
        <Card className="bg-gradient-to-br from-teal-600 to-teal-800 text-white border-2 border-teal-600 hover:border-teal-700 transition-all duration-200 shadow-clinical-lg hover:shadow-2xl relative overflow-hidden h-full flex flex-col justify-between p-6">
          {/* Top accent badge */}
          <div className="flex items-center justify-between gap-2 mb-4">
            ...
            {action.badgeText && (
              <span className="text-[11px] font-bold uppercase tracking-wider bg-white text-teal-600 px-2.5 py-1 rounded-full shadow-sm">
                {action.badgeText}
              </span>
            )}
          </div>
    ...
    <Link href={action.linkUrl} className="block group focus-ring rounded-2xl">
      <Card interactive className="h-full flex flex-col justify-between p-6">
        ...
        <div className="flex flex-col gap-1">
          <h3 className="font-heading font-semibold text-base text-slate-900 group-hover:text-teal-600 transition-colors flex items-center justify-between">
            <span>{action.title}</span>
