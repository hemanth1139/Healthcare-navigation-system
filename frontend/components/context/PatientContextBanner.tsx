...
export const PatientContextBanner: React.FC<PatientContextBannerProps> = ({
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-teal-50 border border-teal-200 text-xs text-teal-700">
    ...
    <Card className="border border-teal-100 bg-teal-50/40 p-4 flex flex-col gap-3">
      ...
      {contextItems.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          ...
          {medications.length > 0 && (
            <div className="flex items-start gap-2 bg-white/80 rounded-lg p-2.5 border border-teal-100">
              <Pill className="w-3.5 h-3.5 text-teal-500 mt-0.5 shrink-0" />
              <div>
                <p className="text-[10px] font-bold text-teal-600 uppercase">Medications</p>
